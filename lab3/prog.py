...
for ii in range(int(h/8)):
    ...
    if k>=razmer : break
    #if (i+j)>N: continue
    DCT_massive[i][j] = int(DCT_massive[i][j] + (0.5 if
    DCT_massive[i][j] > 0 else -0.5))
    if slovo2[k] == 0:
    ...
    for x in range(N):
    for y in range(N):
    obratn[x].append(int(round(DCTobr(DCT_massive, N, x,
    y))))
...
for j in range(N):
DCT_massive[i].append(((DCT(f_massive, N, i, j))))
#вытаскивание сообщения побочная диагональ [i][N-i-1]
if k < razmer:
for i in range(N):
for j in range(N):
if k>=razmer : break
